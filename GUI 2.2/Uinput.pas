...
//Place current input data values in PropList
  ...
  begin
    ...
    //JM 2019-11-07: Add new properties
    if NType = TANKS then
      PropList.Add(aNode.Data[TANK_CANOVERFLOW_INDEX]);

  // Add output values to PropList

//    for v := DEMAND to NODEQUAL do
    for v := DEMAND to NODEVIEWS do
    begin
      if (v IN [DEMANDDEFICIT, PRESSDEFICIT]) and (Ntype <> JUNCS)  then
        Continue;
...
{***  Modified for 2.00.12  ***}
      0:  begin
            for k := FLOW_UNITS_INDEX to STATUS_RPT_INDEX do //Hydraulics
              PropList.Add(Data[k]);
            for k := CHECK_FREQ_INDEX to PRESSURE_EXP_INDEX do
              PropList.Add(Data[k]);
          end;
...
begin
  ...
  begin
    ...
    if k >= 0 then
    begin
      //JM 2019-11-07, editing CanOverflow?
      if k > TANK_SRCQUAL_INDEX then
        k := TANK_CANOVERFLOW_INDEX;
...
{***  Modified for 2.00.12  ***}
    0: Begin
         if I <= STATUS_RPT_INDEX     //HYDRAULICS
         then k := I
         else k := I - STATUS_RPT_INDEX - 1 + CHECK_FREQ_INDEX;

         //JM: vrs 2.2
         if (k <> DEMAND_MODEL_INDEX) and (k >= HEADERROR_INDEX) then
         begin
            Result := Uutils.GetSingle(S,v);      {PDA params}
            case k of
              MIN_PRESSURE_INDEX,
              HEADERROR_INDEX,
              FLOWCHANGE_INDEX: Result := Result and (v >= 0.0);

              HTOL_INDEX..RQTOL_INDEX,
              REQ_PRESSURE_INDEX,
              PRESSURE_EXP_INDEX: Result := Result and (v > 0.0);
            end;
         end;
...
// and map units conversion factors
  ...
  //JM vrs 2.2
  MainForm.FlowUnitsBtn.Caption := TXT_FLOWUNITS_MSG + FlowUnits;
  MainForm.ModelBtn.Caption := TXT_DEMAND_MODEL +
