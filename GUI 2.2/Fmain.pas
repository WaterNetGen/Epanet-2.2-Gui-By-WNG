...
const
  ...
  TXT_MAIN_CAPTION = 'EPANET v2.2 by WNG Team';
  ...
  TXT_SAVE_PROJECT_FILTER = 'Epanet v2.2 (*.NET)|*.NET|' +
   'Epanet v2.0 (*.NET)|*.NET|All files|*.*';
  TXT_NETWORK_FILTER = 'Epanet v2.2 (*.INP)|*.INP|'+
   'Epanet v2.0 (*.INP)|*.INP|All files|*.*';
  ...
  TXT_DEMAND_MODEL = 'Model: ';
  TXT_RESULTS_NONE = 'None';
  TXT_DEFICIENT_NODES = 'DN: ';
  TXT_DEMAND_REDUCTION  = 'DR: ';

//JM: vrs 2.2, run status (status bar)
...
type
  TMainForm = class(TForm)
    ...
    DemandModelMnu: TPopupMenu;
    miModelDDA: TMenuItem;
    miModelPDA: TMenuItem;
    RunImageList: TImageList;
    ResultsBtn: TToolButton;
    ...
    procedure miPDAClick(Sender: TObject);
    ...
    procedure FlowUnitsMnuPopup(Sender: TObject);
    procedure miModelDDAClick(Sender: TObject);
    procedure DemandModelMnuPopup(Sender: TObject);
  private
    ...
    procedure SaveFile(Fname: String; Version :Integer);
...
// Otherwise save project under current file name
  else SaveFile(InputFileName, VERSIONID22);
end;
  ...
  begin
    ...
    begin
      if FilterIndex = 1 then
        SaveFile(Filename, VERSIONID22)
      else
        SaveFile(Filename, VERSIONID2)
    end;
    DefaultExt := '';
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if FilterIndex = 1 then
          Uexport.ExportDataBase(Filename,True, VERSIONID22)
        else
          Uexport.ExportDataBase(Filename,True, VERSIONID2)
      end
    end;
  end;
end;
...
procedure TMainForm.miPDAClick(Sender: TObject);
begin
end;
  ...
  // And place Map form in upper left
    if Assigned(MapForm) then with MapForm do
      SetBounds(0,0,MainForm.ClientWidth - BrowserForm.Width - 4,
         MainForm.ClientHeight - DockHeight - MainForm.StatusBar.Height - 4);
    LockWindowUpdate(0);
  end;
end;
...
procedure TMainForm.SaveFile(Fname: String; Version :Integer);
//------------------------------------------------
...
// Save project under new name
  ...
  begin
    Ufileio.SaveProject(Fname, Version);
    InputFileName := Fname;
    InputFileType := iftNET;
    Caption := Txt_MAIN_CAPTION + ' - ' + ExtractFileName(InputFileName);
    MRUUpdate(Self,InputFileName);
...
procedure TMainForm.DemandModelMnuPopup(Sender: TObject);
//-----------------------------------------------------------------------------
// OnPopup handler for menu attached to the Demand Model Popup
//-----------------------------------------------------------------------------
var i:Integer;
begin
  if Not Assigned(Sender) or Not (Sender IS TPopupMenu) then
    Exit;
  ...
  begin
    ...
    begin
      TmenuItem(Items[I]).Checked := False;
      if SameText(TmenuItem(Items[I]).Caption,
              Network.Options.Data[DEMAND_MODEL_INDEX]) then
        TmenuItem(Items[I]).Checked := True;
    end;
  end;
end;


procedure TMainForm.miModelDDAClick(Sender: TObject);
//-----------------------------------------------------------------------------
// OnClick handler for the menu attached to the Demand Model button
//-----------------------------------------------------------------------------
begin
  ...
  begin
    if not SameText(Caption, Network.Options.Data[DEMAND_MODEL_INDEX]) then
    begin
      Network.Options.Data[DEMAND_MODEL_INDEX] := Caption;
      ModelBtn.Caption := TXT_DEMAND_MODEL +
                 Network.Options.Data[DEMAND_MODEL_INDEX];
      SetChangeFlags;
    end;
  end;
end;
...
begin
  with Sender as TMenuItem do
  begin
    if not SameText(Caption, Network.Options.Data[FLOW_UNITS_INDEX]) then
    begin
      Network.Options.Data[FLOW_UNITS_INDEX] := Caption;
      Uinput.UpdateAllUnits;
      SetChangeFlags;
    end;
  end;
...
procedure TMainForm.FlowUnitsMnuPopup(Sender: TObject);
//---------------------------------------------
// (Called when Flow Units selected.)
// Clears the with Flow Units from USFlowUnits or SIFlowUnits
//---------------------------------------------
var i:Integer;
  mi :TMenuItem;
begin
  FlowUnitsMnu.Items.Clear;

  if UnitSystem = usUS then
  begin
    for i := 0 to High(USFlowUnits) do
    begin
      mi := TMenuItem.Create(FlowUnitsMnu);
      mi.Caption := USFlowUnits[i];
      mi.OnClick := FlowUnitsMnuItemClick;
      if SameText(mi.Caption, Network.Options.Data[FLOW_UNITS_INDEX]) then
        mi.Checked := True;
      FlowUnitsMnu.Items.Add(mi)
    end;
  end
  else
  begin
    for i := 0 to High(SIFlowUnits) do
    begin
      mi := TMenuItem.Create(FlowUnitsMnu);
      mi.Caption := SIFlowUnits[i];
      mi.OnClick := FlowUnitsMnuItemClick;
      if SameText(mi.Caption, Network.Options.Data[FLOW_UNITS_INDEX]) then
        mi.Checked := True;
      FlowUnitsMnu.Items.Add(mi)
...
//JM vrs 2.2, set default "demand model" panel
  ModelBtn.Caption := TXT_DEMAND_MODEL + DefOptions[DEMAND_MODEL_INDEX];
...
begin
  ...
  //JM, Vrs 2.2
  //Temp file to store Demand Deficit values
  TempDDeficitFile := Uutils.GetTempFile(TempDir,'en');
end;
...
begin
  ...
  //JM, Vrs 2.2
  SysUtils.DeleteFile((TempDDeficitFile));
end;
...
procedure TMainForm.ShowResultsStatus;
//-----------------------------------------------------------------------------
// Displays the analysis success or failure icon in the StatusPanel.
//-----------------------------------------------------------------------------
var
...
Begin
 S:=  TXT_RESULTS_NONE;
  if RunStatus in [rsSuccess, rsWarning] then
  begin
    NDefNodes := GetNDeficitNodes(CurrentPeriod);
    S := TXT_DEFICIENT_NODES + IntToStr(NDefNodes);

    Value := GetDeficitDemandPercent(CurrentPeriod);

    S := S + ' ' +  TXT_DEMAND_REDUCTION + Format('%.2f%%', [Value]);


  end;

  ResultsBtn.Caption := S;
End;
