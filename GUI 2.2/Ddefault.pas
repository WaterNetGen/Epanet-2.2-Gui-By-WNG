...
begin
  ...
  2: begin
       ...
       //JM 2019-11-07, v2.2
       for j := CHECK_FREQ_INDEX to PRESSURE_EXP_INDEX do
         PropList[I].Add(Network.Options.Data[j]);
     end;
  end;
end;
...
begin
  ...
  //JM 2019-11-07, get the new defaults
  for j := CHECK_FREQ_INDEX to PRESSURE_EXP_INDEX do
    Network.Options.Data[j] := PropList[2].Strings[j-CHECK_FREQ_INDEX +
                                           (STATUS_RPT_INDEX-FLOW_UNITS_INDEX +1)];
...
// If hydraulics option is left blank, then use factory default
  if TabControl1.TabIndex = 2 then     //HYDRAULICS
  begin
    if Length(S) = 0 then S := DefOptions[Index];

    //JM: let's do some checkings...
    if Index <= STATUS_RPT_INDEX
    then k := Index
    else k := Index - STATUS_RPT_INDEX - 1 + CHECK_FREQ_INDEX;

// Check for valid time value
    if (k >= DURATION_INDEX) and (k <= RPT_START_INDEX) then
      if Uutils.StrHoursToFloat(S) < 0 then IsValid := False;

     //JM: vrs 2.2
     if (k <> DEMAND_MODEL_INDEX) and (k >= HEADERROR_INDEX) then
     begin
        IsValid := Uutils.GetSingle(S,v);      {PDA params}
        case k of
          MIN_PRESSURE_INDEX,
          HEADERROR_INDEX,
          FLOWCHANGE_INDEX: IsValid := IsValid and (v >= 0.0);

          HTOL_INDEX..RQTOL_INDEX,
          REQ_PRESSURE_INDEX,
          PRESSURE_EXP_INDEX: IsValid := IsValid and (v > 0.0);
        end;
     end;
