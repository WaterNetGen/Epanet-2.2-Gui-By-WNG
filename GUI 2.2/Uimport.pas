...
{***  Modified for 2.00.12  ***}
  OptionWords : array[0..29] of PChar =
    ('UNIT',      {0: Flow Units}
     'HEADL',     {1: Headloss Formula}
...
{*** Added for vrs 2.2, JM: 2019-11-07 ***}
     'HEADERROR', {21: Headloss error}
     'FLOWCHANGE',{22: Largest Flow change}
     'HTOL',      {23: HTol}
     'QTOL',      {24: QTol}
     'RQTOL',     {25: RQTol}
     'MODEL',     {26: Demand Model}
     'MINI',      {27: Minimum Pressure}
     'REQ',       {28: Required Pressure}
     'EXP'        {29: Pressure Exponent}
     );
...
begin
  ...
  begin
    ...
    n := Min(Ntoks-1, TANK_VCURVE_INDEX);  //JM: to deal with CanOverflow
    ...
    begin
      if Ntoks > TANK_VCURVE_INDEX then    //CanOverflow?
        aNode.Data[TANK_CANOVERFLOW_INDEX] := TokList[Ntoks-1];

      Network.Lists[TANKS].AddObject(ID, aNode);
    end;
...
// Check which keyword applies
  Result := 0;
  Keyword := TokList[0];
  Index := Uutils.FindKeyWord(Keyword,OptionWords,5);
  case Index of
...
{***  Added for vrs 2.2, JM 2019-11-07  ***}
   21: Network.Options.Data[HEADERROR_INDEX] := TokList[1];
   22: Network.Options.Data[FLOWCHANGE_INDEX] := TokList[1];
   23: Network.Options.Data[HTOL_INDEX] := TokList[1];
   24: Network.Options.Data[QTOL_INDEX] := TokList[1];
   25: Network.Options.Data[RQTOL_INDEX] := TokList[1];
  // PDA options
   26: Network.Options.Data[DEMAND_MODEL_INDEX] := TokList[Ntoks-1];
   27: Network.Options.Data[MIN_PRESSURE_INDEX] := TokList[Ntoks-1];
   28: Network.Options.Data[REQ_PRESSURE_INDEX] := TokList[Ntoks-1];
   29: Network.Options.Data[PRESSURE_EXP_INDEX] := TokList[Ntoks-1];
