...
////    procedure CreateParams(var Params: TCreateParams); override;
  ...
  private
    ...
    procedure GoZoomIn(X: Integer; Y: Integer; bDrawRect :Boolean);
...
procedure TMapForm.FormMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
//--------------------------------------
// Implements the Zoom In/Out on the Zoom
// rectangle which are centered at location X,Y.
//--------------------------------------
var aPoint :TPoint;
begin
...
(*
  if WheelDelta > 0 then
    GoZoomIn(aPoint.X, aPoint.Y)   //Zoom-IN
  else
    GoZoomOut(aPoint.X, aPoint.Y); //Zoom-Out
*)
  With Map.Window Do
    if WheelDelta > 0 then
    Begin
      ZoomRect := Bounds(aPoint.X-Round(Pwidth * 0.45),
                         aPoint.Y-Round(Pheight * 0.45),
                         Round(Pwidth *0.90),
                         Round(Pheight * 0.90));
      GoZoomIn(aPoint.X, aPoint.Y, False)
    End
    Else
      GoZoomOut(aPoint.X, aPoint.Y); //Zoom-Out
...
////  New procedure added to release 2.2
...
procedure TMapForm.GoZoomIn(X: Integer; Y: Integer; bDrawRect :Boolean);
//--------------------------------------
...
// Erase zoom rectangle from map
  ...
  // Exit if can't zoom in anymore
    if (ZoomIndex = High(ZoomFactor))
      or (High(ZoomFactor) >= MAXZOOMRATIO) then Exit; //vrs 2.2
...
//    StatusBarPanel2.Caption := IntToStr(MapZoomRatio) + '%';
    ...
    ZoomLevelLabel.Caption := TXT_ZOOM_LEVEL + IntToStr(MapZoomRatio) + '%';
