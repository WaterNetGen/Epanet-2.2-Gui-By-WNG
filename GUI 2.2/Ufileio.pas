...
type
  ...
  procedure SaveProject(const Fname: String; const Version :Integer);

implementation

uses Fmain, Uimport, Uinput;

//JM: define a set type for the new object properties. It is used to save
//    data in VERSIONID2.

Const MAXSET = 100;
Type TNewObjProperties = set of 0..MAXSET;
...
procedure ReadArray(Reader: TReader; var A: array of String);overload;
var
  i,n: Integer;
begin
...
procedure ReadArray(Reader: TReader; var A: array of String;
                    DefProps: array of String);overload;
//JM: the missing object properties (on VERSIONID3) are filled from
//    the default props array (DefProps)
var
  i,n: Integer;
begin
  n := Reader.ReadInteger - 1;
  ...
  //JM: vrs 2.2
  for i := n+1 to High(DefProps) do   //New props are set to the default value
    A[i] := DefProps[i];

end;


procedure WriteArray(Writer: TWriter; A: array of String);overload;
var
  i,n: Integer;
begin
  n := High(A) + 1;
...
procedure WriteArray(Writer: TWriter; A: array of String; MaxSize :Integer);overload;
//JM: Used to save array for version < VERSIONID3
var
  i,n: Integer;
begin
  n := High(A);
  n := MinIntValue([n, MaxSize]);

  Writer.WriteInteger(n+1);
  for i := 0 to n do Writer.WriteString(A[i]);
end;
...
//      WriteInteger(VERSIONID2);
    ...
    //Write network options to file
      ...
      //JM: vrs 2.2 2019-11-07
//      WriteArray(Writer,Network.Options.Data);
      if (Version < VERSIONID22)  then  //JM: take care about new properties
        WriteArray(Writer,Network.Options.Data, OLD_MAXOPTIONS)
      else
        WriteArray(Writer,Network.Options.Data);
    ...
    //Write reservoirs & tanks to file
      ...
      begin
        ...
        begin
          ...
          //JM: vrs 2.2.
          // take care about TANK_CURVE "*" property
          if (Version < VERSIONID22) and (j = TANKS) then
          begin
            WriteInteger(TANK_VCURVE_INDEX+1);
            for k := 0 to TANK_VCURVE_INDEX-1 do
              WriteString(TNode(slist.Objects[i]).Data[k]);

            s:= TNode(slist.Objects[i]).Data[TANK_VCURVE_INDEX];
            if s = '*' then
              s:= '';
            WriteString(s);
          end
    ...
    // Write default properties to file
      ...
      for i := JUNCS to VALVES do         //JM: vrs 2.2, backward compatibility
      if Version < VERSIONID22 then
        WriteArray(Writer,DefProp[i].Data, OLD_MAXOPTIONS)
      else
    ...
    // Write legend intervals to file
      ...
      begin
        if (Version < VERSIONID22) and (i IN [DEMANDDEFICIT, PRESSDEFICIT]) then
          continue;
...
// Create sorted node list & general purpose string list
  ...
  // Create a Reader object
    ...
    try
      ...
      //Read version ID
        Version := ReadInteger;
        if (Version < VERSIONID1) or (Version > VERSIONID22) then
        begin
          raise EReadError.Create(MSG_READ_ERR);
        end;
      ...
      //Read in network options.
        ...
        //vrs 2.2, JM 2019-11-07
//        ReadArray(Reader,Network.Options.Data);
        ReadArray(Reader,Network.Options.Data, DefOptions);
...
//          ReadArray(Reader,node.Data);
          //JM, vrs 2.2
          ReadArray(Reader,node.Data, DefTank);
      ...
      // Read legend intervals
        for i := ELEVATION to NODEVIEWS do
        begin
          //JM vrs 2.2, Set the default values
          if (Version < VERSIONID22) and (i in [DEMANDDEFICIT, PRESSDEFICIT]) then
            for j := 1 to MAXINTERVALS do
              NodeLegend[i].Intervals[j] :=  NodeVariable[i].DefIntervals[j]
          else
            for j := 1 to MAXINTERVALS do
