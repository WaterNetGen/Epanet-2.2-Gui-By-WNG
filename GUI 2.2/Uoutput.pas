...
//JM, vrs 2.2
function GetNDeficitNodes(const TimePeriod: Integer):Integer;
function GetDeficitDemandPercent(const TimePeriod: Integer):Single;
...
//JM, vrs 2.2. Open Demand Deficit file
  if Result in [rsWarning, rsSuccess] then
  begin
    AssignFile(FDDeficit, TempDDeficitFile);
  {$I-}
    Reset(FDDeficit, 1);
  {$I+}
    if (IOResult <> 0)  then
...
var
  ...
  //vrs 2.2
  newNodeVar :Integer;
  PReq :Single; //PDA: Required/Service pressure
begin

  if NodeVar = DEMANDDEFICIT then  //JM, vrs 2.2
  begin
    p1 := Sizeof(Single) * Nnodes   //Demand Deficit values
          + Sizeof(Single) //Number of Pressure deficient Nodes
          + Sizeof(Single) //Demand Deficit Percent
            ;
    p2 :=  Node*Sizeof(Single);
  end
  else
  begin
    p1 := RecordSize*(Nnodes*NODEVARS + Nlinks*LINKVARS);

    //vrs 2.2, for pressure deficit just read the current pressure
    //
    newNodeVar := NodeVar;
    if NodeVar = PRESSDEFICIT then
      newNodeVar := NodeVariable[PRESSURE].SourceIndex[JUNCS];

    p2 := Offset2 + RecordSize*(Nnodes*(newNodeVar-1) + Node);
  end;

  //vrs 2.2, Required/service pressure
  PReq := StrToFloat(Network.Options.Data[REQ_PRESSURE_INDEX]);
  ...
  begin
    ...
    if NodeVar = DEMANDDEFICIT then
    begin
      Seek(FDDeficit, p3);
      BlockRead(FDDeficit, Value^[i], SizeOf(Single));
    end
    else
    begin
      Seek(FOut, p3);
      ...
      //vrs 2.2, the pressure deficit is the difference between the
      //      current pressure and the required pressure.
      //
      if NodeVar = PRESSDEFICIT then
      begin
        if Value^[i] >= PReq then
          Value^[i] := 0.0         //No deficit, pressure is enougth
        else
          Value^[i] := PReq - Value^[i];
      end;
...
//JM, vrs 2.2
  DDRecSize :Integer;    //Demand Deficit Record Size
  New_Var :Integer;
  PReq :Single;
begin
...
// V is an output variable
  ...
  begin
    //JM, vrs 2.2
    if V = DEMANDDEFICIT then
    begin
      if (RunFlag) and (T < Nperiods) then
      begin
        DDRecSize := Sizeof(Single) * Nnodes   //Demand Deficit values
          + Sizeof(Single) //Number of Pressure deficient Nodes
          + Sizeof(Single) //Demand Deficit Percent
            ;

        p := T*DDRecSize + J*Sizeof(Single);

        Seek(FDDeficit,p);
        BlockRead(FDDeficit,Result,SizeOf(Single));
      end;
      Exit;
    end;
  ...
  // Make sure output results exist
    ...
    // Find index of node & variable in output listing
      ...
      //vrs 2.2
      New_Var := V;
      if V = PRESSDEFICIT then
        New_Var := PRESSURE;
      //vrs 2.2, end

      k := NodeVariable[New_Var].SourceIndex[i];
      if (n >= 0) then
      begin
      ...
      // Determine position of output value in Output file
        ...
        begin
          PReq := StrToFloat(Network.Options.Data[REQ_PRESSURE_INDEX]);
          if Result >= PReq then
            Result := 0.0         //No deficit, pressure is enougth
          else
            Result := PReq - Result;
        end;
        //vrs 2.2, end
...
//JM, vrs 2.2
  ...
  i,j,k :Integer;
begin
  if (NodeVar <> NONE) and (TimePeriod < Nperiods)
     and (RunFlag) then //JM vrs 2.2
  begin
    //JM, vrs 2.2
    if NodeVar = DEMANDDEFICIT then
    begin
      DDRecSize :=   Sizeof(Single) * Nnodes;
      p1 := TimePeriod*(DDRecSize + 2*Sizeof(Single));

      Seek(FDDeficit,p1);
      BlockRead(FDDeficit, Value^, DDRecSize);

      //Set the Demand deficit of the other nodes to zero
      for I := Network.Lists[JUNCS].Count to Nnodes-1 do
        Value^[i] := 0.0;
      Exit;
    end;
    //vrs 2.2 end
    ...
    New_Var := NodeVar;
    if NodeVar = PRESSDEFICIT then
      New_Var := NodeVariable[PRESSURE].SourceIndex[JUNCS];
    //vrs 2.2, end


    p1 := RecordSize*(Nnodes*NODEVARS + Nlinks*LINKVARS);
    p2 := RecordSize*Nnodes*(New_Var-1);
    p3 := Offset2 + (TimePeriod*p1) + p2;
    Seek(Fout, p3);
    BlockRead(Fout, Value^, Nnodes*SizeOf(Single));


    //vrs 2.2, compute pressure deficit
    if NodeVar = PRESSDEFICIT then
    begin
      PReq := StrToFloat(Network.Options.Data[REQ_PRESSURE_INDEX]);
      for i := JUNCS to TANKS do
        ...
        begin
          ...
          if k < 0 then
            continue;
          if Value^[k] >= PReq then
            Value^[k] := 0.0         //No deficit, pressure is enougth
          else
            Value^[k] := PReq - Value^[k];
        end;
    end;
    //vrs 2.2, end
...
function GetNDeficitNodes(const TimePeriod: Integer):Integer;
//----------------------------------------------------
// Get the number of Deficit Nodes in time 'TimePeriod'.
//----------------------------------------------------
var Value :Single;
  p1, p2, p3 :LongInt;
begin
  ...
  if (RunFlag) and (TimePeriod < Nperiods) then
  begin
    p1 := Sizeof(Single) * Nnodes;
    p2 := 2 * Sizeof(Single);

    p3 := TimePeriod*(p1+p2) + p1;

    Seek(FDDeficit,p3);
    BlockRead(FDDeficit,Value,SizeOf(Single));
...
function GetDeficitDemandPercent(const TimePeriod: Integer):Single;
//----------------------------------------------------
// Get the Percentage of Deficit Demand in time 'TimePeriod'.
//----------------------------------------------------
var Value :Single;
  p1, p2, p3 :LongInt;
begin
  ...
  begin
    p1 := Sizeof(Single) * Nnodes;
    p2 := 2 * Sizeof(Single);    //NDefNodes + Demand Deficit Percent

    p3 := TimePeriod*(p1+p2) + p1 + Sizeof(Single);
