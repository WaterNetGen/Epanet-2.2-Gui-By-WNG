...
begin
  try
    ...
    Writeline(S);
    Writeline(ULINE);
    //JM vrs 2.2, include Demand Deficit
    S := Format('%-15s %10s%10s%10s%10s%10s%10s',[TXT_NODE, NodeVariable[DEMAND].Name,
      NodeVariable[DEMANDDEFICIT].Name, NodeVariable[HEAD].Name,
      NodeVariable[PRESSURE].Name,
      NodeVariable[PRESSDEFICIT].Name,
        NodeVariable[NODEQUAL].Name]);
    Writeline(S);
    S := Format('%-15s %10s%10s%10s%10s%10s%10s',[TXT_ID, NodeUnits[DEMAND].Units,
      NodeUnits[DEMANDDEFICIT].Units, NodeUnits[HEAD].Units,
        NodeUnits[PRESSURE].Units,
        NodeUnits[PRESSDEFICIT].Units,
