...
procedure ExportDataBase(const Fname: String; const Withmap: Boolean; Version :integer);
...
begin
  ...
  if (IOResult = 0) then
  try
    if Version = VERSIONID22 then //JM: add a comment to .inp file
      Writeln(F,';== EPANET 2.2 .INP FORMAT FILE ==');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Write(F,Format(' %-16s'#9'%-12s'#9'%-12s'#9'%-12s'#9'%-12s'#9+
            '%-12s'#9'%-12s',
              [slist[i],Data[TANK_ELEV_INDEX],Data[TANK_INITLVL_INDEX],
                Data[TANK_MINLVL_INDEX],Data[TANK_MAXLVL_INDEX],
                  Data[TANK_DIAM_INDEX], s]));


          if Version < VERSIONID22 then  //JM: deal with '*' CurveID
          begin
            if Data[TANK_VCURVE_INDEX] = '*' then
              Write(F,Format(#9'%-16s',['']))
            else
              Write(F,Format(#9'%-16s',[Data[TANK_VCURVE_INDEX]]));
          end
          else
          begin
            if Data[TANK_VCURVE_INDEX] = '' then
            ...
            Write(F,Format(#9'%-3s',[Data[TANK_CANOVERFLOW_INDEX]]));
          end;

          Writeln(F,Format(#9';%s',[Data[COMMENT_INDEX]]));
        end;
      end;
    end;
    Writeln(F);
...
{***  Added for vrs 2.2, JM 2019-11-07  ***}
  ...
  // PDA options
      if Version > VERSIONID2 then
      begin
        Writeln(F,' HeadError        '#9,Data[HEADERROR_INDEX]);
        Writeln(F,' FlowChange       '#9,Data[FLOWCHANGE_INDEX]);
        Writeln(F,' HTOL             '#9,Data[HTOL_INDEX]);
        Writeln(F,' QTOL             '#9,Data[QTOL_INDEX]);
        Writeln(F,' RQTOL            '#9,Data[RQTOL_INDEX]);
